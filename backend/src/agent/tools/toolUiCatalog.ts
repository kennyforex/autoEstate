/**
 * Curated copy for the skill editor Tools tab (description, usage, parameters, examples).
 * Registry ids must match createDefaultRegistry() (excluding execute_skill, ask_clarification).
 */
export interface ToolUiCatalogEntry {
  description: string;
  usage: string;
...
export const TOOL_UI_CATALOG: Record<string, ToolUiCatalogEntry> = {
  contact_lookup: {
    description:
...
export function getToolUiCatalogEntry(toolId: string): ToolUiCatalogEntry | undefined {
  return TOOL_UI_CATALOG[toolId];
}
