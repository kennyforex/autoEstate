...
function buildMenuSummary(data: {
  ...
  products: Array<{
    ...
    optionGroups: Array<{
      name: string;
      pricingMode: string;
      values: Array<{ label: string; effectivePrice: number }>;
    }>;
  }>;
  ...
  const productLines = data.products.map((product) => {
    const baseSummary =
      product.effectiveBasePrice > 0
        ? `base ${product.currency} ${product.effectiveBasePrice}`
        : "";
    const optionSummary = product.optionGroups
      .map((group) => {
        const values = group.values
          .map((value) =>
            group.pricingMode === "delta"
              ? `${value.label} (+${product.currency} ${value.effectivePrice})`
              : `${value.label} (${product.currency} ${value.effectivePrice})`,
          )
          .join(", ");
        return `${group.name} (${group.pricingMode}): ${values}`;
      })
      .join(" | ");

    const combined = [baseSummary, optionSummary].filter(Boolean).join(" | ");
    return `${product.name}${product.category ? ` [${product.category}]` : ""} — ${combined || "No configurable options"}`;
  });
  ...
  return `Menu for ${data.clientGroup.name}${fallbackNote}: ${productLines.join("\n")}.${quoteLine}${validationLine}`;
}
