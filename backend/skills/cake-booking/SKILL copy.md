...
# Column mapping: each field name maps to a column (A, B, C…) in order.
# The LLM passes data using these exact field names; the tool places values in the correct column.
sheetFields:
  - Order ID
  - Order Date
  - Customer
...
**Email is REQUIRED** — you need it to send the confirmation and payment emails. If the customer does not provide an email, you MUST ask for it explicitly. Do NOT proceed to Step 6 without an email address.
...
Use the **`data`** parameter (key-value object). Keys must match the `sheetFields` defined in this skill's frontmatter (case-insensitive). The tool maps each key to the correct column automatically. Missing keys default to `—`.

**Example `data` object (keys must match `sheetFields`):**

```json
{
  "Order ID": "MILLE-20260405-001",
  "Order Date": "2026-04-05", // the current date
  "Customer": "John Chan",
  "Phone": "91234567",
  "Email": "john@example.com",
  "Cake Name": "Layer Cake",
  "Flavor": "Salted Caramel",
  "Size": "10 inch",
  "Servings": "14-18",
  "Pickup Date": "2026-04-10",
  "Pickup Time": "15:00",
  "Decoration Notes": "Happy Birthday",
  "Dietary": "—",
...
- Price and Payment Amount are numeric only — no `$` or `HKD` prefix
- Dates must be `YYYY-MM-DD` format, times in `HH:MM` 24-hour format
- Use `—` (em dash) for empty/unknown fields
...
| **requirements** | Short instruction tied to the schema, e.g. _Extract payment total, currency, date, reference, and doc type for cake order verification. Order ID may appear in reference._ |
...
**action:** `update_row` | **sheetName:** `Cake orders` | **spreadsheetId:** omit | **matchValue:** the same **Order ID**

Use the **`data`** parameter. Only include the columns you want to **change** — unmentioned columns keep their existing values automatically.

**Example `data` object for payment update:**
...
```json
{
  ...
  "Payment Status": "Paid",
  "Payment Amount": "620",
  "Paid Date": "2026-04-05",
  "Payment Checked": "Yes",
  "Receipt": "=HYPERLINK(\"https://drive.google.com/...\",\"Receipt\")"
}
```

- Use `Pending review` for Status/Payment Status if `document_data_capture` failed or was unclear
- **Payment Amount** comes from `document_data_capture` `amount`
- **Paid Date** comes from `document_data_capture` `payDate`, or `—` if empty
