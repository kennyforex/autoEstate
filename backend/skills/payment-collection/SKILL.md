...
## send_whatsapp 電話格式（必讀）

- 工具名：`send_whatsapp`
- 參數 `recipient`：**只填數字**（不要依賴 `+`、空格、橫線；系統會去掉，但 SKILL 內統一用純數字）。
- **香港手機**：試算表 `Phone` 若只有 8 位（例如 `61218051`），發送前必須 **`852` + 8 位** → `recipient` = `85261218051`。
- **已是國碼**：表上若已是 `85261218051`，直接用，不要重複加 `852`。
- **禁止**：只用 `61218051` 當 `recipient`（缺國碼，WhatsApp 可能失敗）。
- **何時才呼叫**：已讀表確認未付，且客戶或員工**明確要求**向該號碼發 WhatsApp。Playground 只模擬，不會真發。

| 表上 Phone | recipient 應填 |
|------------|----------------|
| 61218051 | 85261218051 |
| 85261218051 | 85261218051 |
| +852 6121 8051 | 85261218051 |

### 工具呼叫範例（文字催款）

```json
{
  "recipient": "85261218051",
  "message_type": "text",
  "text": "您好，訂單 CAKE-1042 尚有 HKD 380 待付，請盡快付款，謝謝。"
}
```
...
6. **主動 WhatsApp（可選）**：若試算表 **Phone** 與目前對話聯絡人不同，且客戶或員工明確要求向該號碼發送催款：依上表將 Phone 轉成 `recipient`，呼叫 **send_whatsapp**（`message_type` = `text`，`text` 引用 **Order ID** 與尚欠金額）。向當前對話客戶簡述已發出（電話可遮後四位）。
